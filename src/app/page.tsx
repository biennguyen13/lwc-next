...
export default function Home() {
  const [activeTab, setActiveTab] = useState<
    "candlestick" | "hlc" | "combined" | "bollinger" | "realtime"
  >("candlestick")
  ...
  const [realCandlestickData, setRealCandlestickData] = useState<
    CandlestickData[]
  >([])
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const candlestickData = await fetchBinanceCandlestick(
        symbol,
        interval,
        200
      ) // Giảm xuống 200 items
  ...
  const finalCandlestickData =
    realCandlestickData.length > 0 ? realCandlestickData : candlestickData
  ...
  return (
    <main className="min-h-screen p-2 bg-gray-50 dark:bg-gray-900 transition-colors">
      <div className="max-w-[100vw] mx-auto">
        ...
        <div className="mb-6 md:mb-8 flex gap-2">
          {/* Chart Area */}
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-100 dark:bg-red-900 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-300 rounded-md">
            ...
            <p className="text-xs mt-1">
              Using fallback data. Please check your internet connection and try
              again.
            </p>
          </div>
        )}
